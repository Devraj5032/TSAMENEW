...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await pool
      ...
      .query(`
        SELECT 
          ...
          CONVERT(VARCHAR, ta.entry_date, 108) AS entry_time,  -- Format time as HH:mm:ss
          dl.locality, 
          daa.area, 
          dz.zone,
          CONVERT(VARCHAR, ta.entry_date, 23) AS scanned_date -- Format date as YYYY-MM-DD
        FROM 
    ...
    for (let item of data) {
      ...
      const combinedDateTime = new Date(`${entryDate.toISOString().split("T")[0]}T${entryTime}`);
      ...
      if (item.average_rating < 4) {
        ...
        if (matrixResult.recordset.length > 0) {
          ...
          item.scanned_date = matrixResult.recordset[0].resolved_date;
        }
